...
| field | value |
| --- | --- |
| dorian version | `1.0.0` |
| metric commit | `33e9eaf` (the benchmark figures were measured here, during the release audit) |
| release commit | changes after the metric commit are release-state tooling, docs, and version metadata; the figures below remain the stamped metric-run evidence, not a fresh `1.0.0` benchmark claim |
| Python | 3.12.4 |
...
structural/config-binding paths. The `1.0.0` stamp keeps this current-version doc aligned
