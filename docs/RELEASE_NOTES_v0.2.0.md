...
When an AI agent writes a document, the document may be correct today and
...
See `docs/RELEASE_VALIDATION_REPORT_v0.2.0.md` for the complete evidence.
Reviews were performed by AI review panels executing real commands; no
independent human review is claimed. Headline: all gates passed — lint and
...
- **Benchmark evidence is promising, not public proof.** The headline
  ...
  owner-checked and model-adjudicated; raw artifacts are private, public
  summaries are aggregate-only. A reproducible public micro-benchmark is
...
- **`--extract` is experimental and draft-only.** Measured churn fails its
  ...
  0.20 gate on the public demo doc). Extracted claims are drafts for human
  review, never stable warrant inputs.
