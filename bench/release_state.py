...
Usage:
    ...
    uv run python bench/release_state.py --target 1.0.0 --rc-tag v1.0.0rc2 --json --strict
"""
...
SCHEMA_VERSION = 1
_RC_TAG_RE = re.compile(r"^v(?P<target>\d+\.\d+\.\d+)rc(?P<num>[1-9]\d*)$")
...
def _features_after_rc_fact(facts: dict) -> bool:
    return bool(
        facts["features_after_rc"]
        if "features_after_rc" in facts
        else facts.get("features_after_rc1")
    )
...
def evaluate(
    ...
    *,
    ...
    features_after_rc = _features_after_rc_fact(facts)
    ...
    rc_tag_target_ok = facts.get("rc_tag_target_ok", True)
    version_ok = (
        len(vers) == 1 and None not in vers and facts.get("rc_tag_exists") and rc_tag_target_ok
    )
    states.append(
        _state(
            "S1",
            ...
            [
                ...
                f"exists={facts.get('rc_tag_exists')} target_match={rc_tag_target_ok} "
                f"rc_tag_commit={facts.get('rc_tag_commit')} "
                f"features_after_rc={features_after_rc}"
            ],
            blocker=True,
    ...
    return {
        ...
        "rc_tag": facts.get("rc_tag"),
        "rc_tag_commit": facts.get("rc_tag_commit"),
...
def _decide(states: list[dict], facts: dict, evidence: dict) -> str:
    ...
    features_after_rc = _features_after_rc_fact(facts)
    if features_after_rc or not provenance_ok:
        return "CUT_RC2_READY"  # new behavior after current rc and/or publish path needs a run
...
def render_decision_md(report: dict) -> str:
    ...
    lines = [
        ...
        f"- rc tag: `{report.get('rc_tag')}`",
        f"- features_after_rc: {report.get('features_after_rc')}",
...
def _select_rc_tag(
    ...
    *,
    ...
    """Choose the current RC tag for a final target line.

    `requested` is honored exactly, but a tag outside the target line is marked as a
    target mismatch so S1 halts instead of comparing against the wrong release line.
    Auto-detect considers only tags of the form v<target>rc<number>.
    """
    if requested:
        return requested, _rc_tag_matches_target(requested, target)

    candidates: list[tuple[int, str]] = []
    for tag in tags:
        m = _RC_TAG_RE.fullmatch(tag)
        if not m or m.group("target") != target:
            continue
        candidates.append((int(m.group("num")), tag))
    if not candidates:
        return None, False
    return max(candidates)[1], True


def _rc_tag_matches_target(tag: str, target: str) -> bool:
    m = _RC_TAG_RE.fullmatch(tag)
    return bool(m and m.group("target") == target)
...
def gather_facts(repo: Path, *, target: str = "1.0.0", rc_tag: str | None = None) -> dict:
    ...
    _, all_tags = _git(repo, "tag", "--list")
    selected_rc_tag, rc_tag_target_ok = _select_rc_tag(
        all_tags.split(), target=target, requested=rc_tag
    )
    _, exact_tag = _git(repo, "tag", "--list", selected_rc_tag) if selected_rc_tag else (1, "")
    rc_tag_exists = bool(selected_rc_tag and selected_rc_tag in exact_tag.split())
    _, head = _git(repo, "rev-parse", "HEAD")
    rc_tag_commit = None
    if rc_tag_exists:
        _, rc_tag_commit = _git(repo, "rev-parse", f"{selected_rc_tag}^{{}}")
    rc_anc, _ = (
        _git(repo, "merge-base", "--is-ancestor", selected_rc_tag, "HEAD")
        if rc_tag_exists
        else (1, "")
    )
    features_after_rc = bool(rc_tag_exists and rc_anc == 0 and head != rc_tag_commit)
    ...
    return {
        ...
        "rc_tag": selected_rc_tag,
        "rc_tag_exists": rc_tag_exists,
        "rc_tag_target_ok": rc_tag_target_ok,
        "head_commit": head,
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        "--rc-tag",
        default=None,
        help="current RC tag baseline (default: latest v<target>rcN tag)",
    )
    ...
    facts = gather_facts(repo, target=args.target, rc_tag=args.rc_tag)
